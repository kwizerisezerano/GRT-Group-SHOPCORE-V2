...
function ToggleRow({ title, description, checked, onChange, icon: Icon }: any) {
  return (
    <div className="flex items-center justify-between gap-4 rounded-2xl border bg-white p-3 shadow-sm">
      ...
      <button
        ...
        className={`relative h-6 w-11 shrink-0 rounded-full transition ${checked ? "bg-[#0b3d5c]" : "bg-slate-300"}`}
      >
...
function MiniBar({ label, value, color = "bg-[#0b3d5c]" }: { label: string; value: number; color?: string }) {
  return (
    <div className="rounded-2xl border bg-blue-50/50 p-3">
...
export default function ProfileSettings() {
  ...
  const saveLocalSettings = async () => {
    ...
    try {
      ...
      toast.success(offlineModeActive ? "Profile settings saved offline. They will sync when internet returns." : "Profile settings saved and queued for sync.");
    } catch (error: any) {
  ...
  const renderSection = () => {
    if (activeTab === "business") {
      return (
        <div className="grid gap-4 xl:grid-cols-[1fr_360px]">
          <Card className="rounded-2xl border shadow-sm"><CardContent className="p-4">
            <div className="mb-4 flex items-center gap-3"><Building2 className="h-6 w-6 text-[#0b3d5c]" /><div><h3 className="text-base font-black">Business Identity</h3><p className="text-sm text-slate-600">Core business details used across receipts, reports, EBM, invoices, and dashboards.</p></div></div>
          ...
          <Card className="rounded-3xl border bg-gradient-to-br from-white to-emerald-50 shadow-sm"><CardContent className="p-4">
            <h3 className="font-black">Business Readiness</h3><p className="text-sm text-slate-600">Profile completeness score.</p>
    ...
    if (activeTab === "receipts") {
      return (
        <div className="grid gap-4 xl:grid-cols-[1fr_380px]">
          <Card className="rounded-2xl border shadow-sm"><CardContent className="p-4">
            <div className="mb-4 flex items-center gap-3"><ReceiptText className="h-6 w-6 text-orange-600" /><div><h3 className="text-base font-black">Receipt & Printing Control</h3><p className="text-sm text-slate-600">Prepare thermal receipt behavior and professional branded receipt output.</p></div></div>
            <div className="grid gap-4 md:grid-cols-2">
          ...
          <Card className="rounded-3xl border bg-slate-950 text-white shadow-sm"><CardContent className="p-4">
            <div className="mx-auto max-w-[260px] rounded-3xl bg-white p-4 font-mono text-slate-900 shadow-xl">
    ...
    if (activeTab === "finance") {
      return (
        <Card className="rounded-2xl border shadow-sm"><CardContent className="p-4">
          <div className="mb-4 flex items-center gap-3"><Landmark className="h-6 w-6 text-violet-600" /><div><h3 className="text-base font-black">Finance, Tax & Payment Settings</h3><p className="text-sm text-slate-600">Control currency, VAT, fiscal period, payments, and report visibility.</p></div></div>
    ...
    if (activeTab === "security") {
      return (
        <div className="grid gap-4 xl:grid-cols-[1fr_360px]">
          <Card className="rounded-2xl border shadow-sm"><CardContent className="p-4">
            <div className="mb-4 flex items-center gap-3"><ShieldCheck className="h-6 w-6 text-rose-600" /><div><h3 className="text-base font-black">Security & Access Control</h3><p className="text-sm text-slate-600">Session policy, password rules, 2FA readiness, and audit log controls.</p></div></div>
            <div className="grid gap-4 md:grid-cols-2">
          ...
          <Card className="rounded-3xl border bg-gradient-to-br from-white to-rose-50 shadow-sm"><CardContent className="p-4"><h3 className="font-black">Security Score</h3><div className="mt-4 flex h-28 w-28 items-center justify-center rounded-full border-[10px] border-rose-200 bg-white text-2xl font-black text-rose-700">{readiness.security}%</div><p className="mt-5 text-sm text-slate-600">Enable 2FA and audit logs for stronger enterprise security.</p></CardContent></Card>
        </div>
      );
    }
    ...
    if (activeTab === "offline") {
      return (
        <div className="grid gap-4 xl:grid-cols-[1fr_360px]">
          <Card className="rounded-2xl border shadow-sm"><CardContent className="p-4">
            <div className="mb-4 flex items-center gap-3"><DatabaseBackup className="h-6 w-6 text-emerald-600" /><div><h3 className="text-base font-black">Offline-First & Sync Engine</h3><p className="text-sm text-slate-600">Control offline selling, pending queues, sync behavior, local cache, and backup readiness.</p></div></div>
    ...
    if (activeTab === "ebm") {
      return (
        <div className="space-y-5">
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{ebmItems.map((item) => { const Icon = item.icon; const ready = !!businessSettings[item.key as keyof typeof businessSettings]; return <Card key={item.title} className="rounded-2xl border shadow-sm"><CardContent className="p-4"><div className="flex items-start gap-4"><div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl ${ready ? "bg-emerald-500/10 text-emerald-600" : "bg-slate-950 text-white"}`}><Icon className="h-5 w-5" /></div><div><div className="mb-1 flex items-center gap-2"><h4 className="text-sm font-semibold">{item.title}</h4><Badge variant="outline" className="rounded-full text-[10px]">{ready ? "Ready" : item.status}</Badge></div><p className="text-xs leading-relaxed text-slate-600">{item.description}</p></div></div></CardContent></Card>; })}</div>
        </div>
      );
    }
    ...
    if (activeTab === "display") {
      return (
        <Card className="rounded-2xl border shadow-sm"><CardContent className="p-4">
          <div className="mb-4 flex items-center gap-3"><Palette className="h-6 w-6 text-violet-600" /><div><h3 className="text-base font-black">Language, Appearance & Device Experience</h3><p className="text-sm text-slate-600">Control language readiness, colors, compact layout, and touchscreen POS preference.</p></div></div>
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><TextField label="Primary Language" icon={Languages} value={businessSettings.language} onChange={(v: string) => updateBusinessSetting("language", v)} /><TextField label="Secondary Language" icon={Languages} value={businessSettings.secondary_language} onChange={(v: string) => updateBusinessSetting("secondary_language", v)} /><TextField label="Brand Color" icon={Palette} value={businessSettings.brand_color} onChange={(v: string) => updateBusinessSetting("brand_color", v)} /><TextField label="Accent Color" icon={Palette} value={businessSettings.accent_color} onChange={(v: string) => updateBusinessSetting("accent_color", v)} /></div>
          <div className="mt-5 grid gap-3 md:grid-cols-2"><ToggleRow icon={MonitorSmartphone} title="Touchscreen Mode" description="Optimize POS and key workflows for tablets and touch screens." checked={!!businessSettings.touchscreen_mode} onChange={(v: boolean) => updateBusinessSetting("touchscreen_mode", v)} /><ToggleRow icon={Layers} title="Compact Mode" description="Use denser cards and tables for small screens or high-volume admin work." checked={!!businessSettings.compact_mode} onChange={(v: boolean) => updateBusinessSetting("compact_mode", v)} /></div>
        </CardContent></Card>
      );
    }
    ...
    return (
      <div className="space-y-5">
        ...
        <section>
          <div className="mb-4 flex items-center justify-between gap-3"><div><h3 className="text-lg font-semibold">General Settings</h3><p className="text-sm text-slate-600">Manage the main application configuration.</p></div><Badge variant="outline" className="rounded-full">{pendingCount} planned / pending</Badge></div>
  ...
  return (
    <PageBackground image={warehouseBg} opacity={0.04}>
      <PageShell title="Profile Settings" description="Manage profile preferences, workspace display, security, notifications, offline access, and account-related settings.">
        <div className="space-y-5">
          {(offlineModeActive || pendingSync) && (
            <div className="rounded-3xl border bg-amber-500/10 p-4 text-amber-900 shadow-sm">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div className="flex items-start gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl bg-white/70">{offlineModeActive ? <WifiOff className="h-5 w-5" /> : <UploadCloud className="h-5 w-5" />}</div><div><p className="font-bold">{offlineModeActive ? "Profile settings are using offline cache" : "Profile setting changes waiting to sync"}</p><p className="text-sm opacity-90">Business, receipt, tax, EBM, display, and sync settings can be prepared offline and synced later.</p></div></div><Badge className="w-fit rounded-full bg-white/70 text-amber-900 hover:bg-white/70">{offlineModeActive ? <WifiOff className="mr-1 h-3 w-3" /> : <UploadCloud className="mr-1 h-3 w-3" />}{offlineModeActive ? "Offline Mode" : "Sync Pending"}</Badge></div>
            </div>
          )}
              ...
              <div className="relative flex items-start gap-4">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-2 max-w-3xl text-sm leading-5 text-slate-600">
                    Configure profile preferences, business defaults, receipts, offline sync,
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  placeholder="Search settings, tax, receipts, EBM, security..."
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Button className="rounded-xl bg-blue-600 text-white hover:bg-blue-700" disabled={saving} onClick={saveLocalSettings}>
                  ...
                  {saving ? "Saving..." : offlineModeActive ? "Save Offline" : "Save Profile"}
                </Button>
              </div>
            </div>
          </div>
          ...
          <section>
            <div className="mb-4"><h3 className="text-lg font-semibold">Operational Shortcuts</h3><p className="text-sm text-slate-600">Quick access to important system configuration modules.</p></div>
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
              {[{ title: "Users & Permissions", icon: ShieldCheck, link: "/user-management", helper: "Roles and access" }, { title: "Notifications", icon: Bell, link: "/notifications", helper: "Alerts and notices" }, { title: "Receipt Printing", icon: Printer, tab: "receipts", helper: "Printer preferences" }, { title: "Offline Data", icon: HardDriveDownload, tab: "offline", helper: "Cache and sync" }, { title: "Appearance", icon: Palette, tab: "display", helper: "Theme and layout" }].map((item) => { const Icon = item.icon; return <Card key={item.title} className="cursor-pointer rounded-2xl border shadow-sm transition-all hover:-translate-y-1 hover:shadow-md" onClick={() => item.link ? navigate(item.link) : setActiveTab(item.tab as SettingsTab)}><CardContent className="flex items-center justify-between gap-4 p-4"><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-2xl bg-[#0b3d5c]/10 text-[#0b3d5c]"><Icon className="h-5 w-5" /></div><div><p className="font-semibold">{item.title}</p><p className="text-xs text-slate-600">{item.helper}</p></div></div><ArrowRight className="h-4 w-4 text-slate-600" /></CardContent></Card>; })}
            </div>
          </section>
        </div>
      </PageShell>
